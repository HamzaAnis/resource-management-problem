...
struct Time
{
	...
	Time(int hour = 0, int minute = 0, string arr = "am"){ this->hour = hour, this->minute = minute, this->arr = arr; }
	...
	void displayTime(string massege)
	{
		cout << massege << hour << ":" << minute << " " << arr << endl;
	}
	...
	void setTime(string massege)
	{
		char temp;
		while (true)
		{
			cout << massege;
			if (cin >> hour >> temp >> minute >> arr)
			{
				if (this->validTime())
					break;
